...
export async function updateMercadoPagoSettings(
  ...
  if (input.webhookSecret?.trim()) {
    update.webhook_secret_encrypted = encryptSecret(
...
export async function createMercadoPagoOrder(params: {
  ...
  const payerEmail =
    settings.environment === "test"
      ? "comprador_feliora@testuser.com"
      : params.payer.email;
  const nameParts = (params.payer.firstName?.trim() || "Cliente Feliora")
    .split(/\s+/)
    .filter(Boolean);
  const firstName = nameParts[0];
  const lastName = nameParts.slice(1).join(" ") || "Feliora";
  const payer =
