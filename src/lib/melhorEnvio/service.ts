...
import { assertCouponApplicable } from "@/lib/coupons/service";
import {
  decryptStoredSecret,
  encryptSecret,
...
export async function getMelhorEnvioSettings(): Promise<MelhorEnvioSettingsRow> {
  ...
  if (!data) {
    ...
    if (insertError) {
      throw new Error(
        `Tabela melhor_envio_settings não encontrada. Execute supabase/11_checkout_integrations.sql no Supabase. (${insertError.message})`,
      );
    }
...
export async function createOAuthState(
  ...
  return new SignJWT({ purpose: "melhor_envio_oauth", environment })
...
export async function verifyOAuthState(
  state: string
): Promise<MelhorEnvioEnvironment> {
  const { payload } = await jwtVerify(state, getJwtSecretKey());
  if (payload.purpose !== "melhor_envio_oauth") {
...
export async function buildAuthorizeUrl(): Promise<string> {
  ...
  const state = await createOAuthState(row.environment);
...
export async function exchangeAuthorizationCode(
  ...
  const environment = await verifyOAuthState(state);
  const row = await getMelhorEnvioSettings();
  const redirectUri = (row.redirect_uri || getDefaultRedirectUri()).trim();
...
export async function createCheckoutShippingQuote(
  ...
  const { data: cart, error: cartError } = await createAdminClient()
    .from("carts")
  ...
  const { data: items, error: itemsError } = await createAdminClient()
    .from("cart_items")
    .select(
      "variant_id, product_slug, quantity, unit_price, product_variants ( product_id, products ( width_cm, height_cm, length_cm, weight_kg ) )"
    )
    .eq("cart_id", cart.id);
  if (itemsError) throw new Error(itemsError.message);

  type ItemRow = CheckoutCartItemRow & {
    product_variants?: {
      product_id: number;
      products?: {
  ...
  const cartItems = (items ?? []) as unknown as ItemRow[];
  ...
  const input: ShippingQuoteInput = {
    ...
    products: cartItems.map((item) => {
      ...
      return {
        ...
        height:
          product?.height_cm == null ? undefined : Number(product.height_cm),
        length:
          product?.length_cm == null ? undefined : Number(product.length_cm),
        weight:
          product?.weight_kg == null ? undefined : Number(product.weight_kg),
      };
    }),
  };
  ...
  const { data: quote, error: quoteError } = await createAdminClient()
...
export async function ensurePaidOrderInMelhorEnvioCart(orderId: string) {
  ...
  const { data: orderItems, error: itemsError } = await createAdminClient()
    .from("order_items")
  ...
  const products = (orderItems ?? []).map((item) => ({
    name: String(item.product_name).slice(0, 255),
  ...
  for (let volumeIndex = 0; volumeIndex < groups.length; volumeIndex += 1) {
    await getDb().from("melhor_envio_shipments").upsert(
      {
        order_id: orderId,
    ...
    const { data: claimed, error: claimError } = await createAdminClient()
