...
export function RegisterForm() {
  ...
  const entrarHref =
    next !== "/conta"
      ? `/conta/entrar?next=${encodeURIComponent(next)}`
      : "/conta/entrar";

  return (
    <div className="mx-auto w-full max-w-md">
      <p className="mb-6 text-sm leading-relaxed text-ink-muted">
        Endereço e CPF serão solicitados apenas no checkout. Com Google, o
        telefone pode ser preenchido depois na sua conta.
      </p>
      <div className="space-y-4">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <p className="text-center text-sm text-ink-muted">
            Já tem conta?{" "}
            <Link href={entrarHref} className="text-rose-gold hover:underline">
              Entrar
            </Link>
          </p>
        </form>
