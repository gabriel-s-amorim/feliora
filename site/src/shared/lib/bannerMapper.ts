...
export function mapBanner(row: BannerRow): Banner {
  return {
    id: row.id,
    title: row.title ?? "",
    altText: row.alt_text ?? "Banner Feliora",
    imageUrl: row.image_url,
    imageUrlMobile: row.image_url_mobile,
    linkUrl: row.link_url,
    objectPosition: row.object_position || "center center",
    objectPositionMobile: row.object_position_mobile || "center center",
...
export function bannerToRow(input: BannerInput): Record<string, unknown> {
  return {
    title: input.title ?? "",
    ...
    object_position_mobile: input.objectPositionMobile ?? "center center",
